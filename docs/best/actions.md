...
A downside of inline actions is that TypeScript does not apply type inference on them, so you would have to type all your callbacks.
Instead of creating an action for the entire callback, you could just run the state-modifying part of the callback in an action.
The advantage of this pattern is that it encourages you not to litter the place with `action`, but rather put all the state modifications at the end of the whole process, as much as possible:
...
And after each `await` a new asynchronous function is started, so after each `await`, state-modifying code should be wrapped in an action.
...
The advantage of `flow` is that it is syntactically very close to async / await (with different keywords), and no manual action wrapping is needed for the async parts; resulting in very clean code.

`flow` can only be used as function and not as decorator.
